...
当前已新增 `scripts/update-heat-status.ts`，由 `npm run update-data` 和 GitHub Actions 每日执行。

已自动运行：
- Open-Meteo 未来 16 天天气预报。
- 当柏林预测达到 32°C+ 时开启避暑加权，达到 35°C+ 时显示强避暑触发。
- 自动生成下一次周末短住窗口，默认 3 位成人 + 1 位宝宝/儿童、2 晚。

可选增强：
- `SERPAPI_API_KEY`：通过 SerpApi Google Hotels 查询价格/可订性线索。
- `GOOGLE_PLACES_API_KEY`：通过 Google Places 查询最新排序评论样本，并扫描空调差、房间闷热、亲子不便、泳池/SPA 受限等风险。

没有 API key 时：
- 页面仍显示 Google Hotels 和 Google Maps 检查入口。
- 评论风险使用人工风险基线。
- 页面会明确标注“未配置”，不伪装成已抓取实时价格或近期评论。
